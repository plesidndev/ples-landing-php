...
<section aria-label="Music video" class="w-full py-6 <?= $figmaMobile ? 'px-[25px] md:px-4' : 'px-4' ?> <?= $compactDesktop ? 'md:py-2' : '' ?>">
    ...
    <div class="mx-auto max-w-sm <?= $compactDesktop ? 'md:max-w-[280px]' : '' ?>">
        ...
        <?php endif; ?>
            <?= responsive_img($site['video']['poster'], '', '(min-width: 768px) 384px, 100vw', ['loading' => 'lazy', 'decoding' => 'async', 'class' => 'absolute inset-0 size-full object-cover transition-transform duration-500 group-hover:scale-[1.03]']) ?>
