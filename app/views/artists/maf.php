        ...
        <div class="md:grid md:min-h-screen md:grid-cols-2">
            ...
            <div class="flex min-w-0 flex-col md:min-h-screen md:pt-16">
                <section class="px-3 pb-8 pt-3 text-center md:mx-auto md:w-full md:max-w-xl md:px-10 md:pb-10 md:pt-6">
                    <h1 class="relative mx-auto block h-[88px] w-full max-w-[374px] overflow-hidden md:h-[112px] md:max-w-[480px]"><span class="sr-only">MAF — StarBoy</span><img src="/images/maf/maf-wordmark.png" alt="" width="736" height="1030" decoding="async" class="absolute inset-0 size-full scale-[1.02] object-cover object-[center_50%]"></h1>
                    <p class="mt-1 border border-white/30 py-1 font-mono text-xs text-white">“StarBoy”</p>
                </section>
                <section aria-label="Streaming links" class="px-[18px] pb-6 md:mx-auto md:w-full md:max-w-xl md:px-10"><h2 class="sr-only">Dengarkan <?= e($site['release']) ?> dari <?= e($site['name']) ?></h2><div class="flex flex-col gap-3"><?php foreach ($site['platforms'] as $platform) render('components/dsp-card', compact('site', 'platform')); ?></div></section>
